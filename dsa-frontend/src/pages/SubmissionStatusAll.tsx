...
import { fetchSubmissionList, fetchSubmissionStatus, fetchLectures, fetchMyUserInfo, fetchUserList } from '../api/GetAPI';
...
const SubmissionStatusAll: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllUsers = async () => {
      const data = await apiClient({ apiFunc: fetchUserList, args: [null, null]})
      const dict: {[key: string]: User} = {};
      data.forEach((user) => {
  ...
  return (
    <Paper sx={{ p: 3, margin: 2}}>
      <Typography variant="h4" gutterBottom>全ての提出</Typography>
          ...
          <TableBody>
            {submissions.map((submission) => (
              <TableRow key={submission.id}>
                ...
                <TableCell>{`${userDict[submission.user_id]?.username} (${submission.user_id})`}</TableCell>
...
export default SubmissionStatusAll;
